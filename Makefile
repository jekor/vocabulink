...
sync_options := -avz --exclude 'cgi/dist' --exclude '*.muse' --exclude '*.ccss' --exclude 'articles/Makefile' --exclude '*.el' --exclude 'css/Makefile' --exclude 'js/Makefile' --exclude 'cgi/*.pdf' --exclude 'cgi/TAGS' --exclude '*.aux' --exclude '*.tex' --exclude '*.ptb' --exclude '*.log' --exclude '*.out' --delete articles css etc img js s scripts upload postgresql vocabulink.cgi linode:vocabulink/
