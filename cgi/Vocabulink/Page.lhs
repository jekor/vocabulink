...
>                         currentPage, pager, script) where
...
> import Text.Blaze.Html5 (docTypeHtml, head, script, noscript, link, body, title)
...
>   let  deps' = [CSS "lib.common", JS "lib.common"] ++ (isJust memberNo ? [CSS "lib.member", JS "lib.member"] $ []) ++ deps
...
>     Nothing  -> script ! type_ "text/javascript" $ preEscapedString ("alert('Dependency \"" ++ show d ++ "\" not found.');")
>     Just v   ->
>       case d of
>         CSS  css  -> link ! href (stringValue $ "http://s.vocabulink.com/css/" ++ css ++ ".css?" ++ v)
>                           ! rel "stylesheet" ! type_ "text/css"
>         JS   js   -> script ! src (stringValue $ "http://s.vocabulink.com/js/" ++ js ++ ".js?" ++ v) $ mempty
...
>               Just n  -> reviewBox <$> numLinksToReview n
>   return $ do
>     a ! href "/" ! accesskey "1" $
...
> logoutBox username = form ! class_ "auth-box logout" ! action "/member/logout" ! method "post" $ do
>   string username
>   input ! type_ "submit" ! id "logout-button" ! value "Log Out"
...
>     input ! type_ "text" ! name "q" ! accesskey "s"
>     string " "
>     input ! type_ "submit" ! id "sa" ! name "sa" ! value "Search"
>   script ! src "http://www.google.com/cse/brand?form=cse-search-box&lang=en" $ mempty

n is the number of links due for review (presumably from numLinksToReview)

> reviewBox :: Integer -> Html
